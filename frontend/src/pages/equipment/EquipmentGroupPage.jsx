...
export default function EquipmentGroupPage() {
  ...
  const filteredData = data.filter((d) => {
    ...
    if (activeGroup === "all") return d.ten.toLowerCase().includes(search.toLowerCase());
    ...
    <div className="grid grid-cols-12 gap-4">
      ...
      <div className="col-span-3 space-y-4">
        ...
        <div className="p-3 bg-white rounded-lg shadow space-y-2">
          <h3 className="font-semibold text-sm">Tìm kiếm loại</h3>
          <Input
      ...
      <div className="col-span-9 space-y-3">
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="overflow-x-auto">
            <Table className="min-w-[1000px] border border-gray-200">
              <TableHeader>
                <TableRow className="bg-gray-100 text-sm font-semibold">
                  <TableHead className="text-center border">#</TableHead>
                  <TableHead className="border">Mã thẻ kho</TableHead>
                  <TableHead className="border">Hình ảnh</TableHead>
                  <TableHead className="border">Tên thiết bị</TableHead>
                  <TableHead className="border">Nhóm</TableHead>
                  <TableHead className="border">Tên loại</TableHead>
                  <TableHead className="border">Nhà cung cấp</TableHead>
                  <TableHead className="border">Ngày tạo</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {currentData.map((row, idx) => (
                  <TableRow key={row.id} className="hover:bg-gray-50 text-sm">
                    ...
                    <TableCell className="border">{row.maTheKho}</TableCell>
                    <TableCell className="border">
                      <img src={row.img} alt={row.ten} className="w-12 h-10 object-contain rounded" />
                    </TableCell>
                    <TableCell className="border">{row.ten}</TableCell>
                    <TableCell className="border">{row.nhom}</TableCell>
                    <TableCell className="border italic text-gray-600">{row.loai}</TableCell>
                    <TableCell className="border">{row.nhaCC}</TableCell>
                    <TableCell className="border">{row.ngayNhap}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
              ...
              {Array.from({ length: totalPages }).map((_, i) => (
                <Button
                  ...
                  className={`transition-all ${
                    currentPage === i + 1 ? "bg-emerald-500 text-white font-semibold" : "hover:bg-gray-200"
                  }`}
