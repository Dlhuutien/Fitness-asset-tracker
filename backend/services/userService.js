...
const userService = {
  ...
  createUser: async (data, creatorRole) => {
    ...
    // phân quyền validate
    return await userRepository.adminCreateUser({ ...data, createdByRole: creatorRole });
  },
  ...
  updateUserAttributes: async (data) => {
    if (!data.accessToken || !data.attributes || typeof data.attributes !== "object") {
  ...
  updateUserStatus: async (data) => {
    if (!data.username || typeof data.enabled !== "boolean" || !data.updatedByRole) {
  ...
  adminUpdateUserAttributes: async (data) => {
    if (!data.username || !data.attributes || typeof data.attributes !== "object" || !data.updatedByRole) {
